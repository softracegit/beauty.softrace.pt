(function () {
    ...
    function renderCloseUnpaidPreview(unpaid) {
        ...
        var alertText = count === 1
            ? 'Existe 1 marcação por liquidar ou faturar (últimos 30 dias). Liquide ou fature todas antes de fechar a caixa.'
            : 'Existem ' + count + ' marcações por liquidar ou faturar (últimos 30 dias). Liquide ou fature todas antes de fechar a caixa.';
        if (totalDue > 0.00001) {
    ...
    function setCloseSubmitBlocked(blocked) {
        ...
        if (blocked) {
            submit.setAttribute('title', 'Liquide ou fature todas as marcações dos últimos 30 dias antes de fechar a caixa.');
        } else {
