...
@section('title', 'Editar Oportunidade | Imobiliária')
@section('page-heading-title', 'Editar Oportunidade')
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0">Dados da Oportunidade</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('opportunities.update', $opportunity) }}" method="POST">
                                ...
                                <div class="col-12 col-md-4">
                                    ...
                                    <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
                                        @foreach($statuses as $value => $label)
                                            <option value="{{ $value }}" {{ old('status', $opportunity->status) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                        @endforeach
                                    </select>
                                    @error('status')
                                    ...
                                    @enderror
                                    <div class="mt-2">
                                        <label for="status_change_notes" class="form-label small">Notas sobre mudança de estado (opcional)</label>
                                        <textarea name="status_change_notes" id="status_change_notes" rows="2" class="form-control form-control-sm" placeholder="Adicione uma nota sobre a mudança de estado..."></textarea>
                                    </div>
                                </div>
                                <div class="col-12 col-md-4">
                                    ...
                                    <select name="type" id="type" class="form-select @error('type') is-invalid @enderror" required>
                                        @foreach($types as $value => $label)
                                            <option value="{{ $value }}" {{ old('type', $opportunity->type) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                        @endforeach
                                    </select>
                                    @error('type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                ...
                                <div class="col-12 col-md-4">
                                    ...
                                    <select name="priority" id="priority" class="form-select @error('priority') is-invalid @enderror" required>
                                        @foreach($priorities as $value => $label)
                                            <option value="{{ $value }}" {{ old('priority', $opportunity->priority) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                        @endforeach
                                    </select>
                                    @error('priority')
                                        <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="tab-pane fade" id="relacoes" role="tabpanel">
                            <div class="row g-3">
                                <div class="col-12 col-md-6">
                                    ...
                                    <select name="client_id" id="client_id" class="form-select @error('client_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($clients as $client)
                                            <option value="{{ $client->id }}" {{ old('client_id', $opportunity->client_id) == $client->id ? 'selected' : '' }}>{{ $client->name }} ({{ $client->email }})</option>
                                        @endforeach
                                    </select>
                                    @error('client_id')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                ...
                                <div class="col-12 col-md-6">
                                    ...
                                    <select name="agent_id" id="agent_id" class="form-select @error('agent_id') is-invalid @enderror">
                                        ...
                                        @foreach($agents as $agent)
                                            <option value="{{ $agent->id }}" {{ old('agent_id', $opportunity->agent_id) == $agent->id ? 'selected' : '' }}>{{ $agent->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('agent_id')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea name="notes" id="notes" rows="3" class="form-control @error('notes') is-invalid @enderror">{{ old('notes', $opportunity->notes) }}</textarea>
                                    @error('notes')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                    ...
                    <div class="mt-4 d-flex gap-2 justify-content-end">
                        <a href="{{ route('opportunities.show', $opportunity) }}" class="btn btn-light">Cancelar</a>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Se houver preferência existente, carregar localizações
