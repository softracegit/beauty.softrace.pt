...
@section('definicoes_content')
  <form method="post" action="{{ route('definicoes.marcacoes.update') }}">
    ...
    <div class="card mt-3">
      ...
      <div class="card-body">
        <p class="text-muted small mb-4">
          Define o intervalo dos horários oferecidos no Booking público e o tempo de bloqueio temporário durante o checkout.
        </p>

        <div class="row g-3 align-items-end">
          <div class="col-12 col-md-4 col-lg-3">
            <label for="booking_slot_interval_minutes" class="form-label fw-semibold mb-1">Blocos de horários</label>
            <select
              id="booking_slot_interval_minutes"
              name="booking_slot_interval_minutes"
              class="form-select @error('booking_slot_interval_minutes') is-invalid @enderror"
              required
            >
              @foreach(($bookingSlotIntervalOptions ?? \App\Models\CrmSetting::BOOKING_SLOT_INTERVAL_MINUTES_OPTIONS) as $minutes)
                <option
                  value="{{ $minutes }}"
                  @selected((int) old('booking_slot_interval_minutes', $bookingSlotIntervalMinutes ?? \App\Models\CrmSetting::BOOKING_SLOT_INTERVAL_MINUTES_DEFAULT) === (int) $minutes)
                >
                  De {{ $minutes }} em {{ $minutes }} minutos
                </option>
              @endforeach
            </select>
            @error('booking_slot_interval_minutes')
              <div class="invalid-feedback d-block">{{ $message }}</div>
            @enderror
          </div>
        </div>
        <p class="small text-muted mb-0 mt-2 w-100">
          Define os horários apresentados ao cliente no Booking (ex.: 10:00, 10:30…). A agenda interna e a duração dos serviços não são alteradas; continua a mostrar os horários livres mais próximos dentro deste intervalo.
        </p>
        ...
        <p class="small text-muted mb-0 mt-2 w-100">
          Este tempo é usado no checkout público da marcação para reservar o horário temporariamente.
        </p>
      </div>
    </div>
